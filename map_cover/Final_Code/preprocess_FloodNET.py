...
def data_loading(BASE_DIR, train_ids, val_ids, aug=0, show_samples=False):
    train_paths = [BASE_DIR + '/' + i.rstrip() + '.jpg' for i in train_ids]
    train_dataset = tf.data.Dataset.from_tensor_slices(train_paths)
    train_dataset = train_dataset.map(parse_image)
    # train_dataset = list(map(parse_image, train_ids))

    # val_dataset = list(map(parse_image, val_ids))
    val_paths = [BASE_DIR + '/' + i.rstrip() + '.jpg' for i in val_ids]
