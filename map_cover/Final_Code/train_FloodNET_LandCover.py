...
from Final_Code.preprocess_FloodNET_LandCover import data_loading
import random
...
##Configuration
def train_FloodNET_LandCover(FloodNET_path, LandCover_path, model_name):
    ...
    with strategy.scope():
        ...
        NUM_AUG = 1
        BASE_DIR_FloodNET = FloodNET_path

        images_ids_FloodNET = list(map(lambda x: x[:-4], list(filter(lambda x: x[-5] != 'b', os.listdir(BASE_DIR_FloodNET)))))
        masks_ids_FloodNET = list(map(lambda x: x[:-8] ,list(filter(lambda x: x[-5] == 'b', os.listdir(BASE_DIR_FloodNET)))))
        ids = [id for id in images_ids_FloodNET if id in masks_ids_FloodNET]

        random.shuffle(ids)
        val_size_FloodNET = 2 * len(ids)//10
        train_ids_FloodNET, val_ids_FloodNET = ids[val_size_FloodNET:], ids[:val_size_FloodNET]

        with open('dataset/train.txt') as f:
            train_ids_LandCover = f.readlines()

        with open('dataset/val.txt') as f:
            val_ids_LandCover = f.readlines()

        dataset = data_loading(FloodNET_path, LandCover_path, train_ids_LandCover, val_ids_LandCover,train_ids_FloodNET, val_ids_FloodNET, aug=NUM_AUG)

        train_size = len(train_ids_LandCover) + len(train_ids_FloodNET)
        val_size = len(val_ids_LandCover) + len(val_ids_FloodNET)

        STEPS_PER_EPOCH = (train_size * (2**NUM_AUG)) // BATCH_SIZE
